// sfx.js — the sound of a card table, via Arcade.audio (SDK 3.5.0+).
//
...
// level.
//
// Structure: one registration site (below), one feature-detected play wrapper
// (`sfx`). The launcher owns volume and the global mute, so this game ships no
// sound setting.

// Registered one-shot cues. Order of voices inside an array matters: the noise
// transient always leads, the tone follows a beat later via `delay` so the two
// overlap into a single event rather than reading as two sounds.
const CUES = {
...
// The one registration site — every cue is declared once at module load
// (main.js imports this after the index.html inline Arcade.init has run; audio
// is purely local, so there is no need to await Arcade.ready).
if (audioAvailable()) {
...
// The one guarded play-site, so the Arcade/audio feature check lives in exactly
// one place. `opts` are optional per-play overrides — note that the SDK only
// merges overrides onto single-object cues, so of the cues above only `undo`
// can accept them. Silent + cheap when muted or WebAudio is absent.
